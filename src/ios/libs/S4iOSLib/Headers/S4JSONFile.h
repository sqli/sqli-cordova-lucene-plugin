...
{
@protected
	NSObject											*m_rootJSONObject;
	NSError												*m_lastError;
	NSObject <S4JSONFileDelegate>						*m_delegate;
    BOOL												m_bDoneParsing;
	S4JSONParserOptions									m_parserOptions;
	NSTimeInterval										m_requestTimeout;
	BOOL												m_bUseCache;
	NSOperationQueuePriority							m_queuePriority;
	BOOL												m_bAsyncResponse;
}

// Properties
@property (readonly, strong, nonatomic) NSObject				*rootJSONObject;
@property (readonly, copy, nonatomic) NSError					*lastError;
@property (readwrite, copy, nonatomic) NSString					*reachableHostStr;
@property (readwrite, strong, nonatomic) NSObject				*userContext;

// Class methods
+ (id)jsonFileWithParserOptions: (S4JSONParserOptions)parserOptions;
